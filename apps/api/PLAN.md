# Dugong Backend - High Level Plan
...
**Indexer:** background worker that polls Sui JSON-RPC for `dugong::dugong` events, persists cursor in Postgres, and keeps account records (handles/owners) in sync.
...
- `reqwest` - HTTP client (TwitterAPI.io, X OAuth2, Enclave)
- `serde` - Serialization
...
CREATE TABLE dugong_accounts (
    id SERIAL PRIMARY KEY,
    twitter_user_id VARCHAR(64) NOT NULL UNIQUE,  -- xid
    twitter_handle VARCHAR(64) NOT NULL,
    sui_object_id VARCHAR(66) NOT NULL UNIQUE,    -- DugongAccount ID
...
CREATE TABLE indexer_state (
    id SERIAL PRIMARY KEY,
    name VARCHAR(64) NOT NULL UNIQUE,  -- e.g., "dugong_indexer"
...
2. Fetch full tweet via TwitterAPI.io
3. Parse transfer command:
   "@Dugong send {amount} {coin} to @{receiver}"
4. Validate:
...
```
1. Poll Sui JSON-RPC for dugong::dugong events (by package/module)
2. Track cursor in postgres indexer_state
3. Handle event types:
   - AccountCreated: upsert dugong_accounts (xid, handle, account_id)
...
│       └── models.rs        # DugongAccount, WebhookEvent, IndexerState
└── migrations/
...
DUGONG_PACKAGE_ID=0x...
DUGONG_REGISTRY_ID=0x...

# Enclave
ENCLAVE_URL=http://localhost:43000
...
1. **Fix Enclave**: Debug why enclave rejects tweet URLs (check nautilus-server code)
