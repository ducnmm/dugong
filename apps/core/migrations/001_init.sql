-- ============================================================================
-- Dugong Database Schema
-- Complete initialization with all tables, indexes, and constraints
-- ============================================================================

-- Create dugong_accounts table (matches DugongAccount in smart contract)
CREATE TABLE IF NOT EXISTS dugong_accounts (
...
CREATE INDEX IF NOT EXISTS idx_x_user_id ON dugong_accounts(x_user_id);
CREATE INDEX IF NOT EXISTS idx_x_handle ON dugong_accounts(x_handle);
CREATE INDEX IF NOT EXISTS idx_x_handle_lower ON dugong_accounts(LOWER(x_handle));
CREATE INDEX IF NOT EXISTS idx_sui_object_id ON dugong_accounts(sui_object_id);
...
CREATE TABLE IF NOT EXISTS transfers (
    ...
    from_xid VARCHAR(64),           -- NULL for deposits (external -> dugong)
    to_xid VARCHAR(64),             -- NULL for withdraws (dugong -> external)
    coin_type VARCHAR(256) NOT NULL,
