...
Link a Sui wallet to an X identity reliably and securely: authenticate the caller server-side, verify with a freshly-minted Twitter token, and signal re-authentication cleanly when the X session has expired.
